...
const GroupAnagrams = () => {
    const [anagramObject, setAnagramObject] = useState({});
    ...
    const displayAnagramGroups = (strs) => {
        const newAnagramObject = {};
        for (let s of strs) {
    ...
    return (<div className="pt-8">
      ...
      <button className="p-2 rounded-md bg-blue-500 hover:bg-blue-600" onClick={handleRandomPress}>
        press me for random anagrams
      </button>
      <ul className="flex">
        {testGroup.map((word, index) => (<li className="px-1" key={index}>{word}</li>))}
      </ul>
      ...
      <div>
        <ul>
          {Object.entries(anagramObject).map(([key, group], index) => (<li key={index}>
              <strong>{key}:</strong>
              {group.join(", ")}
            </li>))}
        </ul>
